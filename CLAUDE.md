...
**Draw loop structure** (`gui/draw/`): the drawing code is a directory module split into four files. `draw/mod.rs` owns the orchestration — `GuiState::draw` snapshots mutable fields into locals via `.take()` or `.clone()`, renders via imgui, then writes them back (necessary to avoid borrow conflicts with `imgui::Ui`) — plus renderer init (`init_renderer`, `make_gl`). The rendering logic is split into methods on a local `struct Ctx<'ui> { ui, fw, win_x/y/w/h }` (defined in `draw/widgets.rs`) that carries the shared draw-time state. `draw/widgets.rs` holds `Ctx` and its reusable primitives (`row`, `password_row`, `file_row`, `slider`, `reset_icon_button`, `combo`, `draw_modal_dim`, `fit_label`); `draw/panels.rs` holds the config panels (`connection_fields`, `audio_controls`, `denoise_toggle`, `output_device_pickers`, `mic_picker`, `radio_picker`, `log_level_picker`, `connect_button`, `status_display`) plus the `BrowseClicks` return struct **and the unverified-server confirmation modal** (`confirm_unverified_modal`, the `ConfirmConnect` result enum, the shared `CONFIRM_POPUP_ID`); `draw/file_picker.rs` holds the `##fp` modal (`file_picker_modal`, the `FilePick` result enum, `start_dir`). Primitives and panels are all methods on `Ctx`, declared `pub(super)` so the sibling modules can call across files. The shared `LABEL_COL_X` constant lives in `draw/widgets.rs`.

**Connect confirmation when unverified**: clicking **Connect** with an empty `Server CA` field does *not* connect immediately — `draw/mod.rs` opens the `CONFIRM_POPUP_ID` modal (`confirm_unverified_modal`) warning that the server's certificate won't be verified. Only `ConfirmConnect::Confirm` ("Connect anyway") sets `should_connect`; `Cancel` aborts. Modal-open state is the private `GuiState::confirm_unverified` flag (snapshotted/written back through `draw` like the other fields), and `draw_modal_dim` covers the background while it (or the file picker) is open. With a `Server CA` set, Connect proceeds directly. This is the GUI counterpart to the connect-time warnings emitted in `run_mumble_stack` / the CLI / the status area.
